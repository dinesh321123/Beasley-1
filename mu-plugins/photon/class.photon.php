...
class Beasley_Photon {
	...
	public static function instance() {
		if ( ! is_a( self::$__instance, 'Beasley_Photon' ) ) {
			self::$__instance = new Beasley_Photon;
	...
	private function setup() {
		if ( ! function_exists( 'beasley_photon_url' ) ) {
			return;
		}
	 ...
	 * @uses self::validate_image_url, apply_filters, beasley_photon_url, esc_url
	 * @filter the_content
	 * @return string
	 */
	public static function filter_the_content( $content ) {
		$images = Beasley_Photon::parse_images_from_html( $content );
		if ( ! empty( $images ) ) {
			$content_width = Beasley_Photon::get_content_width();
			...
			foreach ( $images[0] as $index => $tag ) {
				 ...
				 */
				if ( apply_filters( 'beasley_photon_skip_image', false, $src, $tag ) )
					continue;
				...
				if ( self::validate_image_url( $src ) ) {
					...
					if (
						 ...
						 * }
						 */
						apply_filters( 'beasley_photon_image_is_local', false, compact( 'src', 'tag', 'images', 'index' ) )
					) {
					...
					if ( false === $width && false === $height ) {
						list( $width, $height ) = Beasley_Photon::parse_dimensions_from_filename( $src );
					}
					 ...
					 * }
					 */
					$args = apply_filters( 'beasley_photon_post_image_args', $args, compact( 'tag', 'src', 'src_orig', 'width', 'height' ) );
					$photon_url = beasley_photon_url( $src, $args );
					...
					if ( $src != $photon_url ) {
						...
						if ( ! empty( $images['link_url'][ $index ] ) && self::validate_image_url( $images['link_url'][ $index ] ) )
							$new_tag = preg_replace( '#(href=["|\'])' . $images['link_url'][ $index ] . '(["|\'])#i', '\1' . beasley_photon_url( $images['link_url'][ $index ] ) . '\2', $new_tag, 1 );
						...
						if ( isset( $placeholder_src ) && self::validate_image_url( $placeholder_src ) ) {
							$placeholder_src = beasley_photon_url( $placeholder_src );
					...
					$new_tag = preg_replace( '#(href=["|\'])' . $images['link_url'][ $index ] . '(["|\'])#i', '\1' . beasley_photon_url( $images['link_url'][ $index ] ) . '\2', $tag, 1 );
					$content = str_replace( $tag, $new_tag, $content );
				}
			}
		}
	...
	public static function filter_the_galleries( $galleries ) {
		...
		foreach ( $galleries as &$this_gallery ) {
			if ( is_string( $this_gallery ) ) {
				...
				//		$this_gallery['src'] = array_map( 'beasley_photon_url', $this_gallery['src'] );
			}
		}
	...
	public static function filter_the_image_widget( $instance ) {
		if ( ! $instance['attachment_id'] && $instance['url'] ) {
			beasley_photon_url( $instance['url'], array(
				'w' => $instance['width'],
				'h' => $instance['height'],
			) );
		}
	 ...
	 * @uses is_admin, apply_filters, wp_get_attachment_url, self::validate_image_url, this::image_sizes, beasley_photon_url
	...
	public function filter_image_downsize( $image, $attachment_id, $size ) {
		...
		if ( is_admin() &&
		      ...
		      * }
		      */
		     false === apply_filters( 'beasley_photon_admin_allow_image_downsize', false, compact( 'image', 'attachment_id', 'size' ) )
		) {
			return $image;
		}
		/**
		 ...
		 * }
		 */
		if ( apply_filters( 'beasley_photon_override_image_downsize', false, compact( 'image', 'attachment_id', 'size' ) ) ) {
			return $image;
		}
		...
		if ( $image_url ) {
			...
			if ( ( is_string( $size ) || is_int( $size ) ) && array_key_exists( $size, self::image_sizes() ) ) {
				 ...
				 * }
				 */
				$photon_args = apply_filters( 'beasley_photon_image_downsize_string', $photon_args, compact( 'image_args', 'image_url', 'attachment_id', 'size', 'transform' ) );
				// Generate Photon URL
				$image = array(
					beasley_photon_url( $image_url, $photon_args ),
				 ...
				 * }
				 */
				$photon_args = apply_filters( 'beasley_photon_image_downsize_array', $photon_args, compact( 'width', 'height', 'image_url', 'attachment_id' ) );
				// Generate Photon URL
				$image = array(
					beasley_photon_url( $image_url, $photon_args ),
	 ...
	 * @uses self::validate_image_url, beasley_photon_url, Beasley_Photon::parse_from_filename
	 * @uses Beasley_Photon::strip_image_dimensions_maybe, Beasley_Photon::get_content_width
	...
	public function filter_srcset_array( $sources = array(), $size_array = array(), $image_src = array(), $image_meta = array(), $attachment_id = 0 ) {
		...
		foreach ( $sources as $i => $source ) {
			...
			if ( apply_filters( 'beasley_photon_skip_image', false, $source['url'], $source ) ) {
				continue;
			}
			$url = $source['url'];
			list( $width, $height ) = Beasley_Photon::parse_dimensions_from_filename( $url );
			// It's quicker to get the full size with the data we have already, if available
			if ( ! empty( $attachment_id ) ) {
				$url = wp_get_attachment_url( $attachment_id );
			} else {
				$url = Beasley_Photon::strip_image_dimensions_maybe( $url );
			}
			$args = array();
			...
			$sources[ $i ]['url'] = beasley_photon_url( $url, $args );
		}
		/**
		 ...
		 */
		$multipliers = apply_filters( 'beasley_photon_srcset_multipliers', array( 2, 3 ) );
		$url         = trailingslashit( $upload_dir['baseurl'] ) . $image_meta['file'];
		if (
			/** Short-circuit via beasley_photon_srcset_multipliers filter. */
			is_array( $multipliers )
			/** This filter is already documented in class.photon.php */
			&& ! apply_filters( 'beasley_photon_skip_image', false, $url, null )
			...
			if ( abs( $constrained_size[0] - $expected_size[0] ) <= 1 && abs( $constrained_size[1] - $expected_size[1] ) <= 1 ) {
				$crop = 'soft';
				$base = Beasley_Photon::get_content_width() ? Beasley_Photon::get_content_width() : 1000; // Provide a default width if none set by the theme.
			} else {
				$crop = 'hard';
				$base = $reqwidth;
			}
			...
			foreach ( $multipliers as $multiplier ) {
				...
				$newsources[ $newwidth ] = array(
					'url'         => beasley_photon_url( $url, $args ),
					'descriptor'  => 'w',
					'value'       => $newwidth,
				);
			} // foreach ( $multipliers as $multiplier )
	 ...
	 * @uses Beasley_Photon::get_content_width
	...
	protected static function validate_image_url( $url ) {
		...
		if (
			 ...
			 */
			apply_filters( 'beasley_photon_reject_https', false )
		) {
			return false;
		}
	...
	function filter_open_graph_tags( $tags, $parameters ) {
		...
		if ( is_array( $tags['og:image'] ) ) {
			...
			foreach ( $tags['og:image'] as $image ) {
				$images[] = beasley_photon_url( $image, $photon_args );
			}
			$tags['og:image'] = $images;
		} else {
			$tags['og:image'] = beasley_photon_url( $tags['og:image'], $photon_args );
		}
		return $tags;
	}
	...
	public function action_wp_enqueue_scripts() {
		wp_enqueue_script( 'jetpack-photon', plugins_url( 'assets/js/photon.js', __FILE__ ), array( 'jquery' ), 20130122, true );
	}

	public static function get_content_width() {
		$content_width = isset( $GLOBALS['content_width'] ) ? $GLOBALS['content_width'] : false;

		return apply_filters( 'beasley_content_width', $content_width );
	}
}
