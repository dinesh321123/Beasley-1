 ...
 */
function beasley_photon_url( $image_url, $args = array(), $scheme = null ) {
	 ...
	 * @param bool false
	 */
	if ( true === apply_filters( 'beasley_photon_development_mode', false ) ) {
		return $image_url;
	}
	/**
	 ...
	 */
	if ( false !== apply_filters( 'beasley_photon_skip_for_url', false, $image_url, $args, $scheme ) ) {
		return $image_url;
	}
	/**
	 ...
	 * @param string|null $scheme Image scheme. Default to null.
	 */
	$image_url = apply_filters( 'beasley_photon_pre_image_url', $image_url, $args, $scheme );
	/**
	 ...
	 * @param string|null $scheme Image scheme. Default to null.
	 */
	$args = apply_filters( 'beasley_photon_pre_args', $args, $image_url, $scheme );

	if ( empty( $image_url ) ) {
		return $image_url;
	}

	$image_url_parts = @beasley_photon_parse_url( $image_url );
	...
	$custom_photon_url = apply_filters( 'beasley_photon_domain', '', $image_url );
	...
	if (
		...
		|| $image_url_parts['host'] === beasley_photon_parse_url( $custom_photon_url, PHP_URL_HOST )
	) {
		$photon_url = add_query_arg( $args, $image_url );
		return beasley_photon_url_scheme( $photon_url, $scheme );
	}
	/**
	 ...
	 */
	if ( ! apply_filters( 'beasley_photon_any_extension_for_domain', false, $image_url_parts['host'] ) ) {
	 ...
	 */
	$photon_domain = apply_filters( 'beasley_photon_domain', "https://i{$subdomain}.wp.com", $image_url );
	 ...
	 * @param string $image_url_parts['host'] Image URL's host.
	 */
	if ( isset( $image_url_parts['query'] ) && apply_filters( 'beasley_photon_add_query_string_to_domain', false, $image_url_parts['host'] ) ) {
...
add_filter( 'beasley_photon_url', 'beasley_photon_url', 10, 3 );

function beasley_photon_url_scheme( $url, $scheme ) {
 ...
 */
function beasley_photon_parse_url( $url, $component = -1 ) {
	if ( 0 === strpos( $url, '//' ) ) {
...
add_filter( 'beasley_photon_skip_for_url', 'beasley_photon_banned_domains', 9, 2 );
function beasley_photon_banned_domains( $skip, $image_url ) {
	...
	$host = beasley_photon_parse_url( $image_url, PHP_URL_HOST );
...
/**
 * Beasley Photon - Support Text Widgets.
 *
 * @access public
 * @param string $content Content from text widget.
 * @return string
 */
function beasley_photon_support_text_widgets( $content ) {
	if ( class_exists( 'Beasley_Photon' ) ) {
		return Beasley_Photon::filter_the_content( $content );
	}
	return $content;
}
add_filter( 'widget_text', 'beasley_photon_support_text_widgets' );
