   ...
   public static function ietc_imp_exp_init(){
		...
		if(isset($_GET['delete']) && $_GET['delete'] != ''){
			...
			if( !empty($sqlData[0]) ){
				...
				$file_path	= GENERAL_SETTINGS_CPT_DIR_PATH. '/uploads/import-export-tag-category/' . $folderPath . '/'.$fileName;  // path of the file which need to be deleted.
				wp_delete_file( $file_path );
				if(isset($logsName))
				{
					$logsPath	= GENERAL_SETTINGS_CPT_DIR_PATH. '/uploads/import-export-tag-category/logs/'.$logsName;
