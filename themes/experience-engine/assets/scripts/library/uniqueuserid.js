...
export const uniqueUserId = () => {
	let bid_cookie_val = readcookies_method(bid_cookie_key);
	const current_session_cookie_val = readcookies_method(
	...
	if (bid_cookie_val && bid_cookie_val.length === 0) {
		 ...
		 */
		bid_cookie_val = uuidv4();
		backcookie_method(bid_cookie_key, bid_cookie_val);
		backcookie_method(bid_created_cookie_key, Date.now());
	}
	if (bid_cookie_val) {
		sendBidToGA(bid_cookie_val);
	}
